//
//  Owner.swift
//  ch10-1871453-stackView
//
//  Created by SJH on 2022/05/08.
//
...
class Owner{
    ...
    static func loadOwner(sender: UIViewController){ // sender은 present함수를 위해서 필요하다
        ...
        let okAction = UIAlertAction(title: "OK", style: .default, handler: {
            (action) in
            if var owner = alertController.textFields?[0].text{ // 여러 텍스트필드는 배열로 정장되어 있다 우리는 1개이므로
                if owner == "SYSTEM"{owner = "system"}
                Owner.owner = owner
                UserDefaults.standard.set(owner, forKey: "owner")// preferences에 저장한다.
                }
            NotificationCenter.default.post(name: Notification.Name("doneOwner"), object: self)
        })
