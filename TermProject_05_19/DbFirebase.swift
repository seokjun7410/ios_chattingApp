//
//  DBFirebase.swift
//  ch12-1871453-sharedPlan
//
//  Created by SJH on 2022/05/18.
//
...
class DbFirebase: Database {
    ...
    var parentNotification: ((Message?, DbAction?) -> Void)? // PlanGroupViewController에서 설정
    var existQuery: ListenerRegistration? // 이미 설정한 Query의 존재여부
    required init(parentNotification: ((Message?, DbAction?) -> Void)?) {
        self.parentNotification = parentNotification
        reference = Firestore.firestore().collection("message") // 첫번째 "plans"라는 Collection
    }
}
...
let queryReference = reference.whereField("date", isGreaterThanOrEqualTo:
fromDate).whereField("date", isLessThanOrEqualTo: toDate)
...
let plan = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data["data"] as! Data) as? Message
var action: DbAction?
